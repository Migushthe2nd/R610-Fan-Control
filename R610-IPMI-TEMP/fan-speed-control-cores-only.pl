...
sub set_fans_servo {
  my ($ambient_temp, $_coretemps) = (@_);
  my (@coretemps) = @$_coretemps;

  my $weighted_temp = average(@coretemps);
...
while () {
  ...
  @coretemps=`timeout -k 1 20 sysctl -a | grep 'cpu.[[:digit:]]\+.temperature'`;
  ...
  if ($ambient_temp > $default_threshold) {
    ...
    if (!set_fans_servo($ambient_temp, \@coretemps)) {
      # return for next loop without resetting timers and delta change if that fails
      next;
    }
  }
